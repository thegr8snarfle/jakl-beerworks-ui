...
export const Event: React.FC<{ breweryEvent: BreweryEvent; }> = ({ breweryEvent }) => {
  ...
  return (
    <Container className={`${styles.event} my-1 align-items-center`}>
      <Row cols="1">
        <Col>
          <Card className={`${styles.eventCard} m-2 bg-dark text-white shadow-sm`}>
            <Card.Header className="d-flex flex-row gx-2 justify-content-start align-items-center">
              <div className="date h-100 px-2 mr-2 bg-primary">
              ...
              <div className="h-100 d-flex flex-grow-1 align-items-center">
                <h4 className="text-secondary mb-0 ml-2">{breweryEvent.name}</h4>
              </div>
            </Card.Header>
            {/*<img src={breweryEvent.img} />*/}
            <Card.Body className={`${styles.cardBody} pt-0 mb-2 d-flex h-100 flex-column justify-content-between`}>
              <p className="text-white p-0 m-0">
                {breweryEvent.description}
              </p>
              <p className="p-0 m-0 mt-1"><a className="text-secondary fs-6" href="#">See More (link to FB?)</a></p>
            </Card.Body>
          </Card>
        </Col>
      </Row>
